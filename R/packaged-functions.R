...
#' @param considerationradius numeric value of distance, in miles, that you want to consider. Limits the number of geocoded counties. Recommended to be much larger than drivingtime, perhaps twice as large, to ensure that all counties are considered.
#' @param countymaps a spatialpolygonsdataframe. If NA, data is downloaded and read from the US Census
#' @param fipscodes  a data frame of fips codes for all counties with bannerized codes added. If NA, data from the tigris package is modified to work 
...
countieswithindrivingdistance <- function( 
  ...
  drivingcutoff  <- dminutes(drivingtime)
  ...
  countyindex  <- us.map@data$GEOID %in% (nearbycounties %>% filter(withinrange) %>% extract2("GEOID") )
  ...
  pal <- colorBin("Blues"
                  , nearbycounties %>% filter(withinrange) %>% extract2("minutes")
                  , 3, pretty = FALSE)
